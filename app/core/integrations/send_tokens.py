...
def send_bidders_token(bid, coin):
    ...
    if bid.number_of_tokens > coin.number_of_available_token:
...
def coin_bidders(coin):
    """Get coin bidders"""
    bids = models.Bid.objects.filter(coin=coin.id).order_by("-bidding_price")
...
def send_tokens():
    ...
    for coin in coins:
        ...
        if utils.current_date().date() == coin.bidding_window.date():
...
def start_schedular():
    """Start schedular"""
    schedule.every().day.at("00:00").do(send_tokens)
...
def start_thread_bidder_token():
    ...
    send_token = threading.Thread(target=start_schedular)
